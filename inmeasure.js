...
// LOGIKA DI BAWAH INI SEKARANG IDENTIK DENGAN LOGGEDIN.JS
document.addEventListener("DOMContentLoaded", () => {
  ...
  const showLogoutPopup = () => {
    if (logoutPopup && menuOverlay) {
      menuOverlay.classList.add("show"); // Gunakan menuOverlay untuk popup
  ...
  const hideLogoutPopup = () => {
    if (logoutPopup && menuOverlay) {
      menuOverlay.classList.remove("show"); // Gunakan menuOverlay untuk popup
  ...
  menuItems.forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      if (item.classList.contains("has-sub")) {
        ...
        // Jika itu adalah halaman SAAT INI atau '#', cegah reload
        e.preventDefault();
        menuItems.forEach((i) => {
          i.classList.remove("active");
          // Anda salah ketik 'Read:' di file asli, saya perbaiki:
          i.classList.remove("active-sub"); 
        });
  ...
  // Logika di bawah ini (slider, features) tidak ada di inmeasure.html,
  // jadi tidak akan berjalan, tapi tidak masalah.
  // Ini membuat file JS-nya "identik".
